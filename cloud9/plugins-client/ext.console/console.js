...
module.exports = ext.register("ext/console/console", {
    ...
    init: function(){
        ...
        // display open files panel
        panels.activate(require("ext/openfiles/openfiles"));
        
        if (!editors)
            editors = require("ext/editors/editors");
        
        // if hashbang includes file path, then open that file on load
        var filePaths = window.location.hash.substring(1).split("|");
        if (filePaths != "") {
            setTimeout(function() {
                // first close previously open tabs
                tabbehaviors.closealltabs(function() { console.log("closed previous tabs"); });
                
                filePaths.forEach(function(filePath) {
                    var matchTest = /test_(.*)\.py/.test(filePath);
                    if (filePath != "" && !matchTest) {
                        editors.gotoDocument({path: ide.davPrefix + "/refine/" + filePath})
                    }
                });
            }, 1000);
        } else {
